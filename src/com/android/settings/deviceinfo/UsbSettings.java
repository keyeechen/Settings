...
public class UsbSettings extends SettingsPreferenceFragment {
    ...
    private static final String KEY_DIAG_MODEM = "usb_diag_modem";
    private static final String KEY_CHARGE_ONLY = "usb_charge_only";

    private UsbManager mUsbManager;
    private CheckBoxPreference mMtp;
    private CheckBoxPreference mPtp;
    private CheckBoxPreference mDiagModem;
    ...
    private final BroadcastReceiver mStateReceiver = new BroadcastReceiver() {
        public void onReceive(Context content, Intent intent) {
            ...
            if (action.equals(UsbManager.ACTION_USB_STATE)) {
               ...
               if (!mConnected && getActivity()!= null){
                   getActivity().finish();
               }
    ...
    private PreferenceScreen createPreferenceHierarchy() {
        ...
        mChargeOnly = (CheckBoxPreference)root.findPreference(KEY_CHARGE_ONLY);
        ...
        mDiagModem = (CheckBoxPreference)root.findPreference(KEY_DIAG_MODEM);
        if (!getPackageManager().hasSystemFeature(PackageManager.FEATURE_TELEPHONY)
            || SystemProperties.getInt("sys.usb.diagmodem", 0) == 0) {
            root.removePreference(mDiagModem);
        }
        return root;
    }
    ...
    private void updateToggles(String function) {
        if (UsbManager.USB_FUNCTION_MTP.equals(function)) {
            ...
            mDiagModem.setChecked(false);
        } else if (UsbManager.USB_FUNCTION_PTP.equals(function)) {
            mMtp.setChecked(false);
            mPtp.setChecked(true);
            mDiagModem.setChecked(false);
        } else if (containsFunction(function, UsbManager.USB_FUNCTION_RNDIS)
                && containsFunction(function, UsbManager.USB_FUNCTION_MODEM)
                && containsFunction(function, UsbManager.USB_FUNCTION_DIAG)) {
            mMtp.setChecked(false);
            mPtp.setChecked(false);
            mDiagModem.setChecked(true);
        } else  {
            mMtp.setChecked(false);
            mPtp.setChecked(false);
            mDiagModem.setChecked(false);
        ...
        if (UsbManager.USB_FUNCTION_CHARGE_ONLY.equals(function)) {
            mMtp.setChecked(false);
            mPtp.setChecked(false);
            mDiagModem.setChecked(false);
            mChargeOnly.setChecked(true);
        } else {
            mChargeOnly.setChecked(false);
        }
    }
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        ...
        } else if (preference == mDiagModem && mDiagModem.isChecked()) {
            function = new StringBuilder(UsbManager.USB_FUNCTION_RNDIS)
                            .append(',').append(UsbManager.USB_FUNCTION_MODEM)
                            .append(',').append(UsbManager.USB_FUNCTION_DIAG)
                            .toString();
        } else if (preference == mChargeOnly && mChargeOnly.isChecked()) {
