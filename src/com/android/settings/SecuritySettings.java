...
public class SecuritySettings extends RestrictedSettingsFragment
    ...
    private PreferenceScreen createPreferenceHierarchy() {
        ...
        if (!mIsPrimary ||Dsds.hasNoIcc()) {
            root.removePreference(root.findPreference(KEY_SIM_LOCK));
        } else {
            // Disable SIM lock if sim card is missing or unknown
            if (((TelephonyManager.getDefault().getSimStateDs(PhoneConstants.SimId.SIM1.ordinal()) ==
                                 TelephonyManager.SIM_STATE_ABSENT) ||
                (TelephonyManager.getDefault().getSimStateDs(PhoneConstants.SimId.SIM1.ordinal()) ==
                                 TelephonyManager.SIM_STATE_UNKNOWN)) &&
                ((TelephonyManager.getDefault().getSimStateDs(PhoneConstants.SimId.SIM2.ordinal()) ==
                                 TelephonyManager.SIM_STATE_ABSENT) ||
                (TelephonyManager.getDefault().getSimStateDs(PhoneConstants.SimId.SIM2.ordinal()) ==
                                 TelephonyManager.SIM_STATE_UNKNOWN))) {
