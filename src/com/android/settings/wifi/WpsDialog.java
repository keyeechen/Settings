...
public class WpsDialog extends AlertDialog {
    ...
    public static final String WPS_WPA_RECONNECT_ACTION = "com.android.settings.wifi.WpsDialog.WpaReconnect";
    ...
    private void handleEvent(Context context, Intent intent) {
        ...
        if (WifiManager.NETWORK_STATE_CHANGED_ACTION.equals(action)) {
            ...
            /* WPS succeed but failed to connect; continue to connect or not, depends on the User. */
            /* Send a "WPS_WPA_RECONNECT_ACTION" to settings, settings would pop up a dialog, */
            /* with button "cancel/connect/forget" */
            WifiConfiguration network = (WifiConfiguration)intent.getParcelableExtra(
                    WifiManager.EXTRA_WIFI_CONFIGURATION);
            int reason = intent.getIntExtra(WifiManager.EXTRA_CHANGE_REASON, -1);
            if ((reason == WifiManager.CHANGE_REASON_CONFIG_CHANGE) &&
                (network != null) &&
                (network.status == Status.DISABLED) &&
                (network.disableReason == WifiConfiguration.DISABLED_AUTH_FAILURE)) {
                Log.w(TAG, "WPS completed but failed to connect! SSID: " + network.SSID);

                Intent newIntent = new Intent(WPS_WPA_RECONNECT_ACTION);
                newIntent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY);
                newIntent.putExtra(WifiManager.EXTRA_WIFI_CONFIGURATION, network);

                /* Note: Sent Sticky Broadcast here. In case WifiSettings is not ready */
                /* and would miss this message. WifiSettings SHOULD remove this sticky */
                /* broadcast after receive it. */
                mContext.sendStickyBroadcast(newIntent);

                Toast.makeText(
                    mContext,
                    String.format(mContext.getString(R.string.wifi_wps_fail_to_connect),
                        network.SSID),
